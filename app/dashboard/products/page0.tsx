...
export default function ProductsPage() {
  ...
  const openEditDialog = (product: Product) => {
    ...
    setEditProductCategoryId(product.category_id.toString())
    ...
    try {
      const response = await fetch(`${API_ENDPOINTS.products}/${editingProduct.id}`, {
        ...
        body: JSON.stringify({
          ...
          category_id: Number.parseInt(editProductCategoryId),
          filials: editProductFilials,
        }),
      })
  ...
  const handleDeleteProduct = async () => {
    ...
    try {
      const response = await fetch(`${API_ENDPOINTS.products}/${deletingProduct.id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
      ...
      <Dialog open={editDialogOpen} onOpenChange={setEditDialogOpen}>
        <DialogContent className="max-w-md">
          ...
          <form onSubmit={handleEditProduct} className="space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="editName">Mahsulot nomi</Label>
              <Input
                id="editName"
                placeholder="Masalan: Amerikano"
            ...
            <div className="space-y-2">
              <Label htmlFor="editCategory">Kategoriya</Label>
              <Select
                value={editProductCategoryId}
                onValueChange={(value) => setEditProductCategoryId(value)}
              >
                <SelectTrigger>
                  <SelectValue placeholder="Kategoriyani tanlang" />
                </SelectTrigger>
                ...
                {branches.map((branch) => (
                  <div key={branch.id} className="flex items-center space-x-2">
                    <Checkbox
                      ...
                      checked={editProductFilials.includes(branch.id)}
